...
class TransaksiPengeluaranExport implements WithCustomStartCell, FromQuery, WithHeadings, WithEvents, WithStyles, WithColumnWidths
{
    ...
    public function query()
    {
        $query = Transaksipengeluaran::query()
            ->select(
                'tr_pengeluaran.kode',
                'tr_pengeluaran.tanggal_pengeluaran',
                'tr_pengeluaran.jam_pengeluaran',
                'mm_kategori_pengeluaran.nama as kategori',
                'tr_pengeluaran.balance_pengeluaran',
                'tr_pengeluaran.catatan_pengeluaran',
                'mm_jenis_penyimpanan.nama as penyimpanan'
            )
            ->leftJoin('mm_kategori_pengeluaran', 'tr_pengeluaran.id_kategori_pengeluaran', '=', 'mm_kategori_pengeluaran.id')
            ->leftJoin('mm_jenis_penyimpanan', 'tr_pengeluaran.id_jenis_penyimpanan', '=', 'mm_jenis_penyimpanan.id')
        ...
        if (isset($this->tableFilters['Tanggal'])) {
            ...
            if (!empty($dateFilter['start_date'])) {
                $query->whereDate('tanggal_pengeluaran', '>=', $dateFilter['start_date']);
            }

            if (!empty($dateFilter['end_date'])) {
                $query->whereDate('tanggal_pengeluaran', '<=', $dateFilter['end_date']);
            }
        }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $sheet->setCellValue('A1', 'Laporan Transaksi Pengeluaran');
